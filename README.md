...
| ✨ New | **`get_text_keyframes`**: read the CONTENT of every Source Text keyframe, and say whether the text actually changes between them. `get_keyframes` cannot: its `keyValue()` hands back a whole `TextDocument` and reading one throws *«Text document not of Box document type»*, because several fields exist only on box text. Without it the only way to answer "does the copy change over time" is to render frames and look at them, which is how it was done on 24 Aug 2026 |
